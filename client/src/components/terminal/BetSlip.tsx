/**
 * BetSlip – Merged version combining best features from Wildcards and PolyHouse
 *
 * From Wildcards:
 *   - formatUSDC() helper for displaying small fee amounts accurately
 *   - Config loading guard: shouldApplyFee = configLoaded && isFeeCollectionEnabled
 *   - Original stake tracking in onConfirm signature (originalStake param)
 *
 * From PolyHouse:
 *   - Stale odds detection (30-second threshold) with 10-second timer
 *   - Visual "stale" indicator next to odds
 *   - Refresh button highlighting when odds are stale
 *   - Concurrent-fetch guard in retryOrderBook
 *   - Fresh order book recalculation during stale auto-refresh
 *   - CSS variable support for white-label theming
 *
 * New (merged):
 *   - Configurable fee display via showFeeInUI prop/config
 *     When true: show fee breakdown (Wildcards behavior)
 *     When false: hide fee (silent deduction, PolyHouse behavior)
 *   - Configurable points name via pointsName prop (defaults to "WILD")
 */
...
// ---------------------------------------------------------------------------
// From Wildcards: formatUSDC helper
...
// ---------------------------------------------------------------------------
// Fill simulation (shared by both codebases, identical logic)
// ---------------------------------------------------------------------------
...
interface BetSlipProps {
  ...
  odds: number;
  maxBalance: number;
  // From Wildcards: onConfirm includes optional originalStake parameter
  onConfirm: (
    stake: number,
    direction: "yes" | "no",
  ...
  // New (merged): configurable fee display
  showFeeInUI?: boolean;
...
export function BetSlip({
  ...
  // From PolyHouse: stale odds detection state initialized together
  ...
  // -------------------------------------------------------------------------
  // From Wildcards: Fee collection config with loading guard
  ...
  // From PolyHouse: stale odds detection (30-second threshold)
  const STALE_THRESHOLD_MS = 30000;

  // From PolyHouse: update currentTime every 10 seconds for staleness check
  useEffect(() => {
  ...
  // From PolyHouse: guard against concurrent fetches in retry
  ...
  // -------------------------------------------------------------------------
  // From Wildcards: Fee calculation with configLoaded guard
  ...
  // Calculate execution price from order book (3% buffer)
  ...
  // Potential win based on effective bet amount (after fee deduction)
  const potentialWin = effectiveBetAmount * marketOdds;
  // Display adjusted odds showing true return relative to user's stake
  const displayedOdds = stakeNum > 0 ? potentialWin / stakeNum : marketOdds;
  const earnedPoints = Math.floor(stakeNum);
  ...
  // -------------------------------------------------------------------------
  // Confirm handler
  // From PolyHouse: recalculates fresh prices after stale-odds refresh
  // From Wildcards: passes originalStake to onConfirm for fee tracking
  // -------------------------------------------------------------------------
  const handleConfirm = async () => {
    if (stakeNum <= 0) return;

    // From PolyHouse: track fresh prices for recalculation after refresh
    let finalExecutionPrice = executionPrice;
    let finalDisplayedOdds = displayedOdds;

    // From PolyHouse: auto-refresh stale odds before placing bet
    if (isOddsStale && getOrderBook && currentTokenId) {
      ...
      try {
        ...
        if (freshBook) {
          ...
          // From PolyHouse: recalculate execution price from fresh book
          if (freshBook.bestAsk > 0 && freshBook.bestAsk < 0.99) {
            finalExecutionPrice = Math.min(freshBook.bestAsk + PRICE_BUFFER, 0.99);
            const freshMarketOdds = finalExecutionPrice > 0 ? 1 / finalExecutionPrice : 2;
            const freshPotentialWin = effectiveBetAmount * freshMarketOdds;
            finalDisplayedOdds = stakeNum > 0 ? freshPotentialWin / stakeNum : freshMarketOdds;
            console.log("[BetSlip] Updated prices from fresh book:", {
              finalExecutionPrice,
              finalDisplayedOdds,
            });
          }
    ...
    try {
      // From Wildcards: pass effective bet amount AND original stake for fee tracking
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm">
      ...
      >
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            >
              {outcomeLabel}{" "}
              {isSoccer3Way && (
                <span className={betDirection === "yes" ? "text-wild-scout" : "text-wild-brand"}>
                  ({labels[betDirection]})
                </span>
              )}
            </h3>
        ...
        <div className="space-y-4">
          {/* Direction Selection – Only show for soccer 3-way markets */}
          {isSoccer3Way && (
            ...
            <div className="rounded-lg p-3 space-y-1 bg-amber-500/10 border border-amber-500/30">
              ...
              <div className="text-xs text-amber-400/80 space-y-0.5">
                {fillSimulation.noOrderBook ? (
                  ...
                  <p>
                    Order too large – only ${fillSimulation.totalDepth.toFixed(0)} total available ($
                    {fillSimulation.depthAtBestAsk.toFixed(0)} at best price)
                  </p>
                ) : fillSimulation.slippagePercent > 0 ? (
                  <p>
                    Your ${stakeNum} bet will fill at avg price $
                    {fillSimulation.avgPrice.toFixed(3)} (+
                    {fillSimulation.slippagePercent.toFixed(1)}% slippage from $
                    {orderBook?.bestAsk.toFixed(3)})
                  </p>
                ) : (
              ...
              <div>
                ...
                <p
                  className={`text-2xl font-black font-mono ${
                    isOddsStale ? "text-wild-gold/70" : "text-wild-gold"
                  }`}
                >
